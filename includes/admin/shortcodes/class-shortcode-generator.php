...
/**
 * Class RockPress_Shortcode_Generator
 */
abstract class RockPress_Shortcode_Generator {
	...
	public function init() {
		...
		if ( $this->shortcode_tag ) {
			...
			if ( user_can_richedit() ) {
				...
				RockPress_Shortcode_Button::$shortcodes[ $this->shortcode_tag ] = wp_parse_args( $this->shortcode, $defaults );
		...
		if ( $this->validate( $field ) ) {
			...
			foreach ( $field['options'] as $value => $text ) {
				$new_listbox['values'][] = array(
					'text'  => $text,
					'value' => $value,
					'selected' => ( $value === $listbox['value'] ) ? 'true' : 'false',
				);
			}
	...
	/**
	 * Generate a TinyMCE combobox field
	 *
	 * @param array $field
	 *
	 * @return array|false
	 ...
	 * @since 1.0
	 */
	protected function generate_combobox( $field ) {

		$combobox = shortcode_atts( array(
			'label'    => '',
			'minWidth' => '',
			'name'     => false,
			'tooltip'  => '',
		...
		if ( $this->validate( $field ) ) {
			...
			foreach ( $combobox as $key => $value ) {
				...
				if ( 'value' === $key && empty( $value ) ) {
					// $new_combobox[ $key ] = $combobox['name'];
					$new_combobox[ $key ] = $value;
				} elseif ( $value ) {
					$new_combobox[ $key ] = $value;
				}
			}
